...
/*----------------------------------------------------------------------------------
Instructions: Run all of these scripts at once in your Snowflake Account. The total run
 time will be around 5 minutes.
 
Note: In order for these scripts to run you will need to have completed the following:
    Navigate to: Snowsight
        Click: Data
        Click: Private Sharing
        Search: Tasty Bytes
        Click: Tasty Bytes
        Click: Get
            Adjust Database name to: FROSTBYTE_TASTY_BYTES_SETUP_S (All Capitals) 
            Which roles, in addition to ACCOUNTADMIN, can access this database? Public
        Click: Get
      
  Once this is completed, please run all (CTRL + A) of the setup scripts below in full.
 ----------------------------------------------------------------------------------*/
 

USE ROLE accountadmin;
GRANT IMPORTED PRIVILEGES ON DATABASE frostbyte_tasty_bytes_setup_s TO ROLE public;
